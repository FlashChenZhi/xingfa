...
public class Location {
    ...
    public static Location getEmptyLocation(String skuCode) {
        Session session = HibernateUtil.getCurrentSession();
        Query q = session.createQuery("from Location l where  exists( select 1 from Inventory i where l.bay=i.container.location.bay " +
                "and l.level =i.container.location.level  and i.skuCode=:skuCode and i.container.location.seq<l.seq  )  " +
        ...
        if (!q.list().isEmpty()) {
            ...
            q = session.createQuery("select d.job.toLocation from JobDetail d where d.inventory.skuCode=:skuCode and d.job.type=:jobType");
            q.setParameter("skuCode", skuCode);
            ...
            for (String locationNo : locations) {
                ...
                Query lq = session.createQuery("from Location where level=:level and bay=:bay and empty=false and asrsFlag=true  " +
                        "and putawayRestricted=false and reserved=false and seq >:seq order by seq").setMaxResults(1);
            ...
            q = session.createQuery("select l from Location  l,Location l2 where not exists (select 1 from l2 where l.bay=l2.bay " +
                    "and l.level =l2.level  and (l2.empty=false or l2.reserved=true)) and l.empty=true and l.reserved=false " +
                    "and l.asrsFlag = true  and l.putawayRestricted = false order by l.level,l.bay,l.seq");
            if (!q.list().isEmpty()) {
                return (Location) q.list().get(0);
            } else {
