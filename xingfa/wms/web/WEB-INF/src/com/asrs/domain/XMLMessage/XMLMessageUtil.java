...
//        }
    ...
    public static void modifyToSent(String barcode) {
        try {
            ...
            if (m.getType() == XMLMessageType.LOAD_UNIT_AT_ID) {
                ...
                LogWriter.writeInfo(LoggerType.XMLMessageInfo, "Type:" + m.getType() + " Barcode:" + barcode + " is sent and deleted.");
                session.delete(m);
            }
            Transaction.commit();
        } catch (RuntimeException ex) {
    ...
    public static void modifyToAnswered(String barcode) {
        try {
            ...
            LogWriter.writeInfo(LoggerType.XMLMessageInfo, "Type:" + m.getType() + " Barcode:" + barcode + " is answered and deleted.");
            session.delete(m);
            Transaction.commit();
        } catch (RuntimeException ex) {
            Transaction.rollback();
