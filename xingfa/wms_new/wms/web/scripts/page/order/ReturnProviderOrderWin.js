var ReturnProviderOrderWin = function (obj, obj2) {
        var sels = obj;
        var keyword = null;
        var orderNo = obj2;
        ...
        this.orderDetailStore = Ext.create("Ext.data.JsonStore", {
            fields: ['skuCode', 'skuName', 'planQty', 'qty', 'eom']
        });
        ...
        this.init = function () {
            ...
            for (var res in sels) {
                ...
                results.push({inventoryId: data.inventoryId, qty: data.qty, skuCode: data.skuCode});
            }

            Ext.Ajax.request({
                url: 'wms/returnProviderorderAllocate/searchDetail?orderNo=' + orderNo,
                            ...
                            fn: function (btn) {
                                if (btn === 'yes') {
                                    for (var i = 0; i < this.orderDetailStore.getCount(); i++) {
                                        ...
                                        if (data.planQty < data.qty) {
                                            Ext.error(data.skuCode + "分配数量过大");
                                            return;
                                        }
                                    }
                                    ...
                                    Ext.Ajax.request({
                                        url: 'wms/returnProviderorderAllocate/allocation?orderNo=' + orderNo,
                                        headers: {'Content-Type': "application/json; charset=utf-8"},
                                        method: 'POST',
                                        params: Ext.util.JSON.encode(results),
                                        scope: this,
