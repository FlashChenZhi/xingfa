...
public class AsrsJobTest {
    ...
    @Id
    @SequenceGenerator(name = "seq", sequenceName = "SEQ_ASRSJOB_TEST", allocationSize = 1)
    ...
    public boolean equals(Object o) {
        ...
        AsrsJobTest asrsJob = (AsrsJobTest) o;
    ...
    public static AsrsJobTest getById(int id) {
        Session session = HibernateUtil.getCurrentSession();
        return (AsrsJobTest) session.get(AsrsJobTest.class, id);
    }

    public static AsrsJobTest getAsrsJobByMcKey(String mckey) {
        Session session = HibernateUtil.getCurrentSession();
        Query q = session.createQuery("from AsrsJob aj where aj.mcKey=:mckey").setString("mckey", mckey);
        return (AsrsJobTest) q.uniqueResult();
    }
